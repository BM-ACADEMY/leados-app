...
export const AllianceInboxView = () => {
  ...
  const { inbox: leads, loading: loadingLeads, refetch: refetchLeadsList } = useAllianceInbox();
  const [activeLeadId, setActiveLeadId] = useState(null);
  const { lead: activeLead, conversations, refetch: refetchLead, loading: loadingLead } = useAllianceLead(activeLeadId);
  ...
  const handleSend = async (e) => {
    ...
    setSending(true);
    try {
      await api.sendAllianceMessage(activeLeadId, msg);
      setMsg('');
      refetchLead();
    } catch (err) {
  ...
  const displayLeads = (leads || []).filter(l => l.name.toLowerCase().includes(search.toLowerCase()));
