...
const PAGE_SIZE = 25;
const STATUS_CLASS = { pending: 'new', scheduled: 'seq', paused: 'rep', sent: 'int', failed: 'rep', stopped: 'rep', completed: 'int', in_sequence: 'seq' };
...
export const EmailCampaignDetail = ({ campaignId, onClose }) => {
  ...
  return <div className="al-wa-modal-backdrop" role="presentation" onMouseDown={onClose}>
    <div className="al-wa-detail" role="dialog" aria-modal="true" onMouseDown={(event) => event.stopPropagation()}>
      ...
      {campaign && <div className="al-wa-detail-stats"><div><b>{campaign.prospects}</b><span>Recipients</span></div><div><b>{campaign.sent}</b><span>Sent emails</span></div><div><b>{campaign.replied}</b><span>Replied leads</span></div><div><b>{campaign.failed}</b><span>Failed</span></div><div><b>{templates.length}</b><span>Touches</span></div></div>}
      {!!touchStats.length && <div className="al-wa-detail-note">{touchStats.map((item) => `Touch ${item.touch_no}: ${item.sent} sent · ${item.pending} pending · ${item.failed} failed`).join('   |   ')}</div>}
      <div className="al-wa-detail-filters"><div className="al-input-icon"><Search size={15}/><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search name, business, or email" /></div><select value={status} onChange={(event) => setStatus(event.target.value)}><option value="">All statuses</option>{['pending','in_sequence','scheduled','paused','sent','failed','stopped','completed'].map((item) => <option key={item} value={item}>{item.replaceAll('_', ' ')}</option>)}</select><span>{total} lead{total === 1 ? '' : 's'}</span></div>
